...
def parse_formula(formula, ns, action):
    if (formula.tag == "{%s}is-fireable" % ns):
        transitions = []        
        for transition in formula.findall(".//{%s}transition" % ns):
            transitions.append("%s??\"%s\"" % (action, transition.text))
        return " || ".join(transitions)
    if (formula.tag == "{%s}negation" % ns):        
        s = parse_formula(formula[0], ns, action)
        return "!(%s)" % s  
    if (formula.tag == "{%s}conjunction" % ns):        
        sl = parse_formula(formula[0], ns, action)
        sr = parse_formula(formula[1], ns, action)
        return "(%s) && (%s)" % (sl, sr)
    ...
    if (formula.tag == "{%s}all-paths" % ns):
        return "A(%s)" % parse_formula(formula[0], ns, action)
    if (formula.tag == "{%s}exists-path" % ns):
        return "E(%s)" % parse_formula(formula[0], ns, action)
    if (formula.tag == "{%s}globally" % ns):
        return "[](%s)" % parse_formula(formula[0], ns, action)
    if (formula.tag == "{%s}finally" % ns):
        return "<>(%s)" % parse_formula(formula[0], ns, action)
    if (formula.tag == "{%s}next" % ns):
        return "X(%s)" % parse_formula(formula[0], ns, action)
    if (formula.tag == "{%s}until" % ns):
        ...
        sl = parse_formula(formula[0][0], ns, action)
        sr = parse_formula(formula[1][0], ns, action)
        return "(%s) U (%s)" % (sl, sr)    
...
def main():
    ...
    if (args.category == "deadlock"):
        ...
        types = []
        for idx, formula in enumerate(formulas):
            (invariant, type) = parse_reach(formula, ns, args.action)
            types.append(type)
        ...
        if args.backend == 'sym':
            print(" && timeout %d %s %s --inv-par" % (args.timeout, command_sym, " ".join(invariants)))
        elif args.backend == 'mc':
        ...
        opts = []
        for idx, formula in enumerate(formulas):
            ltl = parse_formula(formula[0], ns, args.action)
            if (ltl[0:1] == "A"):
        ...
        for i, name in enumerate(names):
            ...
            print(" ; timeout $(( %d / %d )) %s %s --buchi-type=spotba --strategy=ufscc --ltl-semantics=spin --state=tree" % (args.timeout, len(names), command_mc, opts[i]), end="")
        ...
        if (args.backend != 'sym'):
            ...
            sys.exit(1)
        for idx, formula in enumerate(formulas):
            ctl = parse_formula(formula[0], ns, args.action)
        ...
        print(" && timeout %d %s %s --mu-par --mu-opt" % (args.timeout, command_sym, " ".join(ctls)))
